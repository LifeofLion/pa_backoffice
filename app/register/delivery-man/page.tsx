...
export default function DeliveryManRegisterPage() {
  ...
  const checkUserStatus = async () => {
    try {
      ...
      console.log('🔍 Current user ID toString():', currentUser.id.toString())
      
      // 🔍 DEBUG: Vérifier spécifiquement le rôle livreur
      console.log('🔍 currentUser.livreur:', currentUser.livreur)
      console.log('🔍 !!currentUser.livreur:', !!currentUser.livreur)
      console.log('🔍 hasOwnProperty livreur:', currentUser.hasOwnProperty('livreur'))
      ...
      if (hasRole) {
        ...
        const cleanUserId = String(currentUser.id).split(':')[0].split('.')[0]
        console.log('🔧 Clean user ID:', cleanUserId)
        
        const justifications = await apiClient.get<JustificationPiecesResponse>(API_ROUTES.JUSTIFICATION.BY_USER(cleanUserId))
        ...
        console.log('🔍 Array.isArray(justifications.data):', Array.isArray(justifications.data))
        if (justifications.data) {
          console.log('🔍 justifications.data.length:', justifications.data.length)
          justifications.data.forEach((j: JustificationPiece, index: number) => {
        ...
        if (hasValidatedDocs) {
          console.log(`✅ User documents validated, redirecting to /app_deliveryman`)
  ...
  const checkDocumentsValidated = (justifications: JustificationPiecesResponse): boolean => {
    try {
      const data = Array.isArray(justifications.data) ? justifications.data : 
                   Array.isArray(justifications) ? justifications as JustificationPiece[] : []
      
      console.log('🔍 DEBUG checkDocumentsValidated - Raw data:', data)
      
      // Debug: voir la structure exacte des premiers documents
      if (data.length > 0) {
        console.log('🔍 Premier document structure:', Object.keys(data[0]))
      ...
      const userJustifications = data.filter((j: any) => {
        ...
        console.log(`🔍 Document ${j.id} - accountType: ${accountType} (from account_type: ${j.account_type}, accountType: ${j.accountType})`)
        return accountType === 'livreur'
      })
      
      console.log('🔍 Justifications filtrées pour livreur:', userJustifications.length)
