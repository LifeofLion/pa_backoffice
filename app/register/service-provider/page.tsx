...
export default function ServiceProviderRegisterPage() {
	...
	const checkUserStatus = async () => {
		try {
			...
			if (hasRole) {
				...
				if (hasValidatedDocs) {
					console.log(
						`✅ User documents validated, redirecting to /app_service-provider`
					);
				...
				console.log(
					`ℹ️ User doesn't have service-provider role, showing form`
				);
				setShowForm(true);
			}
		} catch (error) {
	...
	const checkDocumentsValidated = (justifications: any): boolean => {
		try {
			...
			console.log(
				'🔍 Justifications filtrées pour prestataire:',
