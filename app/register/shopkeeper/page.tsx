...
export default function ShopkeeperRegisterPage() {
  ...
  const checkUserStatus = async () => {
    try {
      ...
      if (hasRole) {
        ...
        if (hasValidatedDocs) {
          console.log(`✅ User documents validated, redirecting to /app_shopkeeper`)
        ...
        console.log(`ℹ️ User doesn't have shopkeeper role, showing form`)
        setShowForm(true)
      }
  ...
  const checkDocumentsValidated = (justifications: any): boolean => {
    try {
      ...
      console.log('🔍 Justifications filtrées pour commercant:', userJustifications.length)
