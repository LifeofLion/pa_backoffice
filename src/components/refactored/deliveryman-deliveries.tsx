...
export default function DeliverymanDeliveries() {
	...
	useEffect(() => {
		if (myLivraisons && myLivraisons.length > 0) {
			const formattedDeliveries: Delivery[] = myLivraisons.map(
				(livraison) => {
					let status:
						| 'paid'
						| 'in_transit'
					...
					const deliveryItem: Delivery = {
						id: livraison.id,
						announceName:
							livraison.annonce?.title ||
							livraison.colis?.[0]?.description ||
							`Livraison #${livraison.id}`,
						image: '/placeholder.svg',
						whereTo:
							livraison.dropoffLocation || 'Adresse de livraison',
						pickupLocation:
							livraison.pickupLocation || 'Point de retrait',
						price: livraison.amount
							? `€${livraison.amount}`
							: 'Prix non spécifié',
						amount: 1,
	...
	useEffect(() => {
		if (availableLivraisons && availableLivraisons.length > 0) {
			const formattedAvailable: Delivery[] = availableLivraisons.map(
				(livraison) => {
					const deliveryItem: Delivery = {
						id: livraison.id,
	...
	const currentDeliveries =
		activeTab === 'assigned' ? deliveries : availableDeliveries;
			...
			<div className='min-h-screen flex items-center justify-center bg-gray-50'>
				<div className='text-center'>
					...
					<p className='text-gray-600 mb-4'>
						Cette page est uniquement accessible aux livreurs
						inscrits.
					</p>
					<Link
						href='/app_client'
										...
										<tr>
											...
											>
												{activeTab === 'available'
													? 'Annonce'
													: t(
															'deliveryman.deliveriess.announceName'
													  )}
											</th>
											<th
												scope='col'
											...
											>
												{t(
													'deliveryman.deliveriess.whereTo'
												)}
											</th>
											<th
												scope='col'
											...
											>
												{activeTab === 'available'
													? 'Date souhaitée'
													: t(
															'deliveryman.deliveriess.deliveryDate'
													  )}
											</th>
											<th
												scope='col'
											...
											>
												{t(
													'deliveryman.deliveriess.status'
												)}
											</th>
											{activeTab === 'available' && (
												<th
												...
												(delivery) => (
													<tr
														...
														onClick={
															...
															'assigned'
																? () =>
																		(window.location.href = `/app_deliveryman/delivery/${delivery.id}`)
																: undefined
														}
													>
											...
											<tr>
												...
												>
													{activeTab === 'available'
														...
														: t(
																'deliveryman.deliveriess.noDeliveriesFound'
														  )}
												</td>
											</tr>
										)}
