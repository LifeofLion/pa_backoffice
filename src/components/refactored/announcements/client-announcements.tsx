...
export default function ClientAnnouncements() {
	...
	useEffect(() => {
		const fetchAnnouncements = async () => {
			...
			try {
				const response = await apiClient.getUserAnnouncements(
					user.id.toString()
				);
	...
	const handleCreateShoppingList = async (e: React.FormEvent) => {
		...
		try {
			...
			const response: any = await apiClient.createAnnouncement(
				announcementData
			);
						...
						>
							...
							<div>
								...
								<input
									...
									onChange={(e) =>
										setShopDeliveryAddress(e.target.value)
									}
									className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500'
									required
								/>
