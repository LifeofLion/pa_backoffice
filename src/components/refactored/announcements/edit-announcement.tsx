		...
		const loadAnnouncement = async () => {
			...
			try {
				...
				if (announcementData.utilisateurId !== user.id) {
					console.log(
						'❌ Authorization failed - Not the announcement owner'
					);
...
export default function EditAnnouncement() {
	...
	// Hooks personnalisés
	const { announcement, loading, error, updateAnnouncement, clearError } =
		useAnnouncementEdit(id as string);

	const { image, imagePreview, handleImageChange, setInitialImagePreview } =
		useImageUpload();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const updateData: UpdateAnnouncementRequest = {
				...
				price:
					typeof announcement.price === 'string'
				...
				actual_delivery_date:
					announcement.actualDeliveryDate || undefined,
				...
				insurance_amount:
					typeof announcement.insuranceAmount === 'string'
						? parseFloat(announcement.insuranceAmount)
			...
			if (updateData.start_location || updateData.end_location) {
				try {
					console.log(
						'🔄 Synchronisation des livraisons associées...'
					);
					...
					const livraisonsResponse = await apiClient.get(
						`/annonces/${announcement.id}/livraisons`
					);
					const livraisons =
						(livraisonsResponse as any)?.livraisons ||
						(livraisonsResponse as any)?.data ||
						[];

					console.log(
						`📦 Trouvé ${livraisons.length} livraison(s) à synchroniser`
					);

					// Mettre à jour chaque livraison avec les nouvelles adresses
					for (const livraison of livraisons) {
						console.log(
							`🔍 Livraison ${livraison.id} - Status: ${livraison.status}`
						);
						...
						if (
							...
							const updatePayload = {
								livreur_id:
									livraison.livreurId || livraison.livreur_id,
								pickup_location: updateData.start_location,
								dropoff_location: updateData.end_location,
								estimated_delivery_time:
									updateData.desired_date,
								status: livraison.status,
							};

							console.log(
								`📤 Mise à jour livraison ${livraison.id} avec:`,
								updatePayload
							);
							...
							console.log(
								`✅ Livraison ${livraison.id} mise à jour`
							);
						} else {
							console.log(
								`⏭️ Livraison ${livraison.id} ignorée (status: ${livraison.status})`
							);
						}
					}
					...
					console.error(
						'⚠️ Erreur synchronisation livraisons (non bloquant):',
						syncError
					);
				}
			}
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								<div>
									...
									<select
										...
										className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500'
									>
										<option value='transport_colis'>
											{t('announcements.transportColis')}
										</option>
										<option value='service_personne'>
											{t('announcements.servicePersonne')}
										</option>
									</select>
								</div>
