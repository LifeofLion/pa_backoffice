  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    packages.forEach((pkg, index) => {
      ...
      if (!pkg.price || pkg.price <= 0) {
        errors.push(`Le Montant de la livraison ${index + 1} doit être supérieur à 0`)
      }
      if (!pkg.weight || pkg.weight <= 0) {
        errors.push(`Le poids du colis ${index + 1} doit être supérieur à 0`)
      }
      if (!pkg.packageSize) {
    ...
    try {
      ...
      const announcementData: CreateAnnouncementRequest = {
        ...
        start_location: startingType === 'address' ? startingAddress : `Boîte de stockage: ${startingBox}`,
        desired_date: deliveryDate,
        priority: packages.some(pkg => pkg.priorityShipping),
        storage_box_id: startingType === 'box' ? startingBox : undefined,
        tags: packages.map(pkg => pkg.packageSize),
        insurance_amount: 0
      }

      console.log("🏠 DEBUG ADRESSES ENVOYÉES AU BACKEND:")
      console.log("📍 Adresse de destination:", destinationAddress)
      console.log("📍 Adresse de départ:", startingType === 'address' ? startingAddress : `Boîte de stockage: ${startingBox}`)
      ...
      const validationErrors = FrontendValidators.validateCreateAnnouncementRequest(announcementData)
      if (validationErrors.length > 0) {
        console.log("❌ Erreurs de validation frontend:", validationErrors)
      ...
      const response: any = await apiClient.createAnnouncementWithImage(announcementData, firstPackageImage)
      ...
      for (let i = 0; i < packages.length; i++) {
        ...
        try {
          const colisData = {
            ...
            width: pkg.packageSize === 'Small' ? 15 : pkg.packageSize === 'Medium' ? 20 : 25,
            height: pkg.packageSize === 'Small' ? 10 : pkg.packageSize === 'Medium' ? 15 : 20,
          ...
          console.error(`❌ Erreur création colis ${i + 1}:`, colisError)
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            id={`weight-${index}`}
                            value={pkg.weight}
                            onChange={(e) => handlePackageChange(index, 'weight', Number(e.target.value))}
                            min="0"
                            step="0.1"
                            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
                          />
