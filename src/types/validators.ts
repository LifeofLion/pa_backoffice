...
export interface StripeInvoiceRequest {
  ...
  items: Array<{                   // vine.array(vine.object(...))
...
export interface ClientWalletTransaction {
  id: number
  client_id: number
  type_transaction: 'recharge' | 'payment' | 'gain' | 'virement' | 'commission'
  montant: number
  solde_avant: number
  solde_apres: number
  description: string
...
export interface WalletTransactionData {
  ...
  utilisateur_id: number
  type_transaction: 'credit' | 'debit' | 'liberation' | 'virement' | 'commission'
  montant: number
  solde_avant: number
  solde_apres: number
  description: string
...
export class FrontendValidators {
  ...
  static validateStripeSubscribeRequest(data: StripeSubscribeRequest): string[] {
    const errors: string[] = []
  ...
  static validateStripePaymentIntentRequest(data: StripePaymentIntentRequest): string[] {
    const errors: string[] = []
    ...
    if (data.currency && !['eur', 'usd', 'gbp'].includes(data.currency.toLowerCase())) {
  ...
  static validateStripeConnectedAccountRequest(data: StripeConnectedAccountRequest): string[] {
    const errors: string[] = []
    ...
    if (data.accountType && !['individual', 'company'].includes(data.accountType)) {
      errors.push('Type de compte invalide')
    }
  ...
  static validateStripeDeliveryPaymentRequest(data: StripeDeliveryPaymentRequest): string[] {
    const errors: string[] = []
    ...
    if (!data.livreurStripeAccountId || data.livreurStripeAccountId.trim().length === 0) {
  ...
  static validateStripeServicePaymentRequest(data: StripeServicePaymentRequest): string[] {
    const errors: string[] = []
    ...
    if (!data.prestataireFeeAccountId || data.prestataireFeeAccountId.trim().length === 0) {
  ...
  static validateStripeTransferRequest(data: StripeTransferRequest): string[] {
    const errors: string[] = []
    ...
    if (!data.destinationAccountId || data.destinationAccountId.trim().length === 0) {
  ...
  static validatePaymentStatus(status: string): status is PaymentStatus {
    return ['unpaid', 'authorized', 'pending', 'paid', 'cancelled', 'refunded'].includes(status)
  }
  ...
  static validateDeliveryStatus(status: string): status is DeliveryStatus {
    return ['pending', 'accepted', 'in_transit', 'delivered', 'completed', 'cancelled'].includes(status)
  }
  ...
  static validateIBAN(iban: string): string[] {
    ...
    if (!/^[A-Z]{2}\d{2}[A-Z0-9]{4}\d{7}([A-Z0-9]?){0,16}$/.test(cleanIban)) {
   ...
   */
  static validateTransferAmount(amount: number, availableBalance: number): string[] {
    const errors: string[] = []
   ...
   */
  static validateStripeEscrowPaymentRequest(data: StripeEscrowPaymentRequest): string[] {
    const errors: string[] = []
    ...
    if (!data.description || data.description.length < 3 || data.description.length > 255) {
   ...
   */
  static validateCodeValidationRequest(data: CodeValidationRequest): string[] {
    const errors: string[] = []
    ...
    if (data.user_info.startsWith('delivery-')) {
      ...
      if (!livraisonIdFromUserInfo || isNaN(parseInt(livraisonIdFromUserInfo))) {
   ...
   */
  static validateStripeConnectOnboardingRequest(data: StripeConnectOnboardingRequest): string[] {
    const errors: string[] = []
   ...
   */
  static validateStripeConnectTransferRequest(data: StripeConnectTransferRequest): string[] {
    const errors: string[] = []
   ...
   */
  static validateStripeConnectPayoutConfigRequest(data: StripeConnectPayoutConfigRequest): string[] {
    const errors: string[] = []
    ...
    if (data.delayDays !== undefined && (data.delayDays < 0 || data.delayDays > 30)) {
   ...
   */
  static validateWalletTransferRequest(data: WalletTransferRequest): string[] {
    const errors: string[] = []
   ...
   */
  static validateClientWalletRechargeRequest(data: ClientWalletRechargeRequest): string[] {
    const errors: string[] = []
   ...
   */
  static validateClientWalletPaymentRequest(data: ClientWalletPaymentRequest): string[] {
    const errors: string[] = []
   ...
   */
  static validateClientWalletMixedPaymentRequest(data: ClientWalletMixedPaymentRequest): string[] {
    const errors: string[] = []
    ...
    if (Math.abs((data.montant_cagnotte + data.montant_carte) - data.montant_total) > 0.01) {
      errors.push('La somme des montants cagnotte et carte doit égaler le montant total')
    }
   ...
   */
  static validateClientWalletBalance(montantDemande: number, soldeDisponible: number): string[] {
    const errors: string[] = []
    
    if (montantDemande > soldeDisponible) {
      errors.push(`Solde insuffisant. Disponible: ${soldeDisponible.toFixed(2)}€, Demandé: ${montantDemande.toFixed(2)}€`)
    }
...
export class ServiceValidators {
  ...
  static validateServiceRequest(data: ServiceRequest): string[] {
    ...
    if (!data.name || data.name.length < 3 || data.name.length > 100) {
      errors.push('Le nom du service doit faire entre 3 et 100 caractères')
    }
  ...
  static validateServiceTypeRequest(data: ServiceTypeRequest): string[] {
    ...
    if (data.description && (data.description.length < 5 || data.description.length > 255)) {
...
export class BookingValidators {
   ...
   */
  static validateBookingStatusUpdate(data: BookingStatusUpdateRequest): string[] {
    const errors: string[] = []
  ...
  static validateBookingFilters(filters: BookingFilters): string[] {
    ...
    if (filters.status && filters.status.some(s => !['pending', 'confirmed', 'in_progress', 'completed', 'cancelled'].includes(s))) {
      errors.push('Statut de filtre invalide')
    }
    
    if (filters.date_start && filters.date_end && new Date(filters.date_start) > new Date(filters.date_end)) {
...
export class RatingValidators {
  ...
  static validateAdminRatingResponse(data: AdminRatingResponse): string[] {
    ...
    if (data.admin_response && data.admin_response.length > 1000) {
      errors.push('La réponse admin ne peut pas dépasser 1000 caractères')
    }
  ...
  static validateRatingFilters(filters: RatingFilters): string[] {
    ...
    if (filters.rating_type && !['service', 'delivery', 'prestataire', 'livreur'].includes(filters.rating_type)) {
