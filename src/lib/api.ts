...
import { API_CONFIG, WALLET_ROUTES, STRIPE_ROUTES, AUTH_ROUTES, USER_ROUTES, ANNOUNCEMENT_ROUTES, DELIVERY_ROUTES, COMPLAINT_ROUTES, MESSAGE_ROUTES, SERVICE_ROUTES, SERVICE_TYPE_ROUTES, PACKAGE_ROUTES, TRACKING_ROUTES } from './api-routes'
...
class ApiClient {
  ...
  async login(email: string, password: string): Promise<{ token: string; user: User }> {
    const response = await this.post<{ token: string; user: User }>(API_ROUTES.AUTH.LOGIN, {
      email,
      password,
    })
  ...
  async register(userData: any): Promise<{ token: string; user: User }> {
    const response = await this.post<{ token: string; user: User }>(API_ROUTES.AUTH.REGISTER, userData)
  ...
  async getUserAnnouncements(userId: string): Promise<{ annonces: any[] }> {
    return this.get<{ annonces: any[] }>(API_ROUTES.ANNOUNCEMENT.BY_USER(userId))
  }
  ...
  async createAnnouncement(data: any): Promise<{ annonce: any }> {
    return this.post<{ annonce: any }>(API_ROUTES.ANNOUNCEMENT.CREATE, data)
  }

  async createAnnouncementWithImage(data: any, imageFile?: File): Promise<{ annonce: any }> {
    ...
    return this.uploadFile<{ annonce: any }>(API_ROUTES.ANNOUNCEMENT.CREATE, formData)
  }

  async updateAnnouncement(id: string, data: any): Promise<{ annonce: any }> {
    return this.put<{ annonce: any }>(API_ROUTES.ANNOUNCEMENT.UPDATE(id), data)
  }

  async updateAnnouncementWithImage(id: string, data: any, imageFile?: File): Promise<{ annonce: any }> {
    // Si pas d'image, utiliser la méthode JSON normale
    if (!imageFile) {
    ...
    return this.uploadFile<{ annonce: any }>(API_ROUTES.ANNOUNCEMENT.UPDATE(id), formData)
  }
  ...
  async getClientDeliveries(clientId: string): Promise<any[]> {
    console.log('🔍 API call: getClientDeliveries with clientId:', clientId)
    console.log('🔍 URL construite:', API_ROUTES.DELIVERY.BY_CLIENT(clientId))
    
    const response = await this.get<any>(API_ROUTES.DELIVERY.BY_CLIENT(clientId))
    console.log('🔍 Raw API response:', response)
    console.log('🔍 Response structure:', JSON.stringify(response, null, 2))
    
    // Le backend retourne { livraisons: { data: [...] } } avec pagination
    if (response.livraisons?.data && Array.isArray(response.livraisons.data)) {
      console.log('✅ Found livraisons.data with length:', response.livraisons.data.length)
      return response.livraisons.data
    }
    // Si pas de pagination, retourner directement le tableau
    if (Array.isArray(response.livraisons)) {
      console.log('✅ Found livraisons array with length:', response.livraisons.length)
      return response.livraisons
    }
    // Nouvelle vérification: si response.success et response.data
    if (response.success && response.data?.data) {
      console.log('✅ Found success response with data.data:', response.data.data.length)
  ...
  async getComplaints(): Promise<any[]> {
    const response = await this.get<{ complaints: any[] }>(API_ROUTES.COMPLAINT.INDEX)
    return Array.isArray(response.complaints) ? response.complaints : []
  }

  async getComplaint(id: string): Promise<any> {
    const response = await this.get<{ complaint: any }>(API_ROUTES.COMPLAINT.BY_ID(id))
    return response.complaint
  }

  async getUserComplaints(userId: string): Promise<any[]> {
    const response = await this.get<{ complaints: any[] }>(API_ROUTES.COMPLAINT.BY_USER(userId))
    return Array.isArray(response.complaints) ? response.complaints : []
  }

  async createComplaint(data: any): Promise<any> {
    const response = await this.post<{ complaint: any }>(API_ROUTES.COMPLAINT.CREATE, data)
    return response.complaint
  }

  async updateComplaint(id: string, data: any): Promise<any> {
    const response = await this.put<{ complaint: any }>(API_ROUTES.COMPLAINT.UPDATE(id), data)
    return response.complaint
  }
  ...
  async getConversations(): Promise<{ conversations: any[] }> {
    return this.get<{ conversations: any[] }>(API_ROUTES.MESSAGE.CONVERSATIONS)
  }
  ...
  async createService(data: any): Promise<any> {
    ...
    const servicePayload = {
      ...
      availability_description: data.availability_description || 'Disponible sur demande',
      status: 'pending',
      is_active: false,
    }
  ...
  async getAdminServices(filters?: any): Promise<any> {
    ...
    if (filters) {
      ...
      if (filters.service_type_id) params.append('service_type_id', Array.isArray(filters.service_type_id) ? filters.service_type_id.join(',') : filters.service_type_id)
  ...
  async getBookings(filters?: any): Promise<any> {
    ...
    if (filters) {
      ...
      if (filters.prestataire_id) params.append('prestataire_id', filters.prestataire_id.toString())
      ...
      if (filters.payment_status) params.append('payment_status', Array.isArray(filters.payment_status) ? filters.payment_status.join(',') : filters.payment_status)
  ...
  async updateBookingStatus(id: string, data: { status: string; notes?: string }): Promise<any> {
  ...
  async getAllRatingsAdmin(filters?: any): Promise<any> {
    ...
    if (filters) {
      const params = new URLSearchParams()
      if (filters.rating_type) params.append('rating_type', Array.isArray(filters.rating_type) ? filters.rating_type.join(',') : filters.rating_type)
      if (filters.rating) params.append('rating', Array.isArray(filters.rating) ? filters.rating.join(',') : filters.rating)
      if (filters.is_visible !== undefined) params.append('is_visible', filters.is_visible.toString())
      if (filters.has_admin_response !== undefined) params.append('has_admin_response', filters.has_admin_response.toString())
      if (filters.date_start) params.append('date_start', filters.date_start)
      if (filters.date_end) params.append('date_end', filters.date_end)
  ...
  async addAdminRatingResponse(ratingId: string, response: string): Promise<any> {
    return this.post<any>(`/ratings/${ratingId}/admin-response`, { admin_response: response })
  }
  ...
  async getPackageLocationHistory(trackingNumber: string): Promise<any[]> {
    return this.get<any[]>(API_ROUTES.PACKAGE.LOCATION_HISTORY(trackingNumber))
  }

  async updatePackageLocation(trackingNumber: string, locationData: any): Promise<any> {
    return this.patch<any>(API_ROUTES.PACKAGE.UPDATE_LOCATION(trackingNumber), locationData)
  }
  ...
  async getLivraisonTracking(livraisonId: string): Promise<any> {
    return this.get<any>(API_ROUTES.TRACKING.LIVRAISON_TRACKING(livraisonId))
  }

  async getLivreurPositions(livreurId: string, options?: {
    startDate?: string,
    endDate?: string,
    livraisonId?: string
  }): Promise<any> {
    ...
    if (options) {
      ...
      if (options.livraisonId) params.append('livraison_id', options.livraisonId)
  ...
  async getRealTimePackageLocation(trackingNumber: string): Promise<any> {
    return this.get<any>(API_ROUTES.PACKAGE.REAL_TIME_LOCATION(trackingNumber))
  }
  ...
  async subscribeUser(data: { planId: string; priceId: string }): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.SUBSCRIBE, data)
  }

  async cancelSubscription(subscriptionId: string): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.CANCEL_SUBSCRIPTION, { subscriptionId })
  }
  ...
  async updateSubscription(subscriptionId: string, data: any): Promise<any> {
    return this.put<any>(API_ROUTES.STRIPE.UPDATE_SUBSCRIPTION(subscriptionId), data)
  }
  ...
  async confirmPayment(data: { paymentIntentId: string; paymentMethodId?: string }): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.CONFIRM_PAYMENT, data)
  }

  async capturePayment(paymentIntentId: string): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.CAPTURE_PAYMENT, { paymentIntentId })
  }

  async refundPayment(data: { paymentIntentId: string; amount?: number; reason?: string }): Promise<any> {
  ...
  async getOnboardingLink(accountId: string): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.GET_ONBOARDING_LINK, { accountId })
  }
  ...
  async createExpressDashboardLink(accountId: string): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.CREATE_EXPRESS_DASHBOARD_LINK(accountId), {})
  }
  ...
  async createStripeConnectAccount(): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CREATE_ACCOUNT, {})
  }
  ...
  async createStripeConnectOnboardingLink(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_ONBOARDING_LINK, data)
  }
  ...
  async transferFromWalletToBank(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_TRANSFER_FROM_WALLET, data)
  }
  ...
  async configureStripeConnectPayouts(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CONFIGURE_PAYOUTS, data)
  }
  ...
  async downloadInvoice(invoiceId: string): Promise<{ success: boolean; download_url: string }> {
    return this.get<{ success: boolean; download_url: string }>(`/stripe/invoice/${invoiceId}/download`)
  }

  // 🚀 NOUVEAU: Générer et télécharger une facture pour une livraison
  async downloadDeliveryInvoice(deliveryId: string): Promise<{ success: boolean; download_url?: string; invoice_data?: any }> {
  ...
  async createRating(ratingData: {
    ...
    return this.post<{ success: boolean; message: string; rating: any }>('/ratings', ratingData)
  }
  ...
  async getItemRatings(type: 'delivery' | 'service', itemId: string): Promise<{
    success: boolean
    ratings: any[]
    average_rating: number
    total_ratings: number
  ...
  async checkUserRating(type: 'delivery' | 'service', itemId: string): Promise<{
    success: boolean
    has_rated: boolean
    rating?: {
      id: number
  ...
  async checkPassword(password: string): Promise<boolean> {
    const response = await this.post<{ valid: boolean }>(API_ROUTES.USER.CHECK_PASSWORD, { password })
  ...
  async configureAutoTransfer(userId: string, data: {
    iban: string
    bic: string
    seuil: number
  }): Promise<any> {
    return this.post<any>(API_ROUTES.WALLET.CONFIGURE_AUTO_TRANSFER(userId), data)
  }

  async disableAutoTransfer(userId: string): Promise<any> {
    return this.post<any>(API_ROUTES.WALLET.DISABLE_AUTO_TRANSFER(userId), {})
  }

  async requestTransfer(userId: string, data: {
    montant: number
    iban: string
    bic: string
  }): Promise<any> {
  ...
  async confirmerRechargeCagnotte(data: { payment_intent_id: string }): Promise<any> {
    return this.post<any>(API_ROUTES.WALLET.CONFIRMER_RECHARGE_CAGNOTTE, data)
  }
  ...
  async createClientStripeConnectAccount(): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CLIENT_CREATE_ACCOUNT, {})
  }
  ...
  async createClientStripeConnectOnboardingLink(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CLIENT_ONBOARDING_LINK, data)
  }

  // Vérifier le statut du compte Stripe Connect client
  async getClientStripeConnectAccountStatus(): Promise<any> {
    return this.get<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CLIENT_ACCOUNT_STATUS)
  }
  ...
  async transferFromClientWalletToBank(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CLIENT_TRANSFER_FROM_WALLET, data)
  }

  // Créer un lien vers le dashboard Stripe Express pour client
  async createClientStripeConnectDashboardLink(): Promise<any> {
    return this.post<any>(API_ROUTES.STRIPE.STRIPE_CONNECT_CLIENT_DASHBOARD_LINK, {})
  }
  ...
  async createLivraisonPaymentWithWallet(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.CREATE_LIVRAISON_PAYMENT_WITH_WALLET, data)
  }
  ...
  async createServicePaymentWithWallet(data: {
    ...
    return this.post<any>(API_ROUTES.STRIPE.CREATE_SERVICE_PAYMENT_WITH_WALLET, data)
  }
...
export function getErrorMessage(error: unknown): string {
  if (isApiError(error)) {
    ...
    if (error.data && typeof error.data === 'object') {
      const dataMessage = error.data.message || 
                         ...
                         error.data.error ||
                         (Array.isArray(error.data.errors) ? error.data.errors.join(', ') : null)
  ...
  if (error && typeof error === 'object') {
    try {
      ...
      return errorObj.message || errorObj.error_message || JSON.stringify(error)
    } catch {
