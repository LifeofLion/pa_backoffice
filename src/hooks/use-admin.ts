...
export function useAdmin() {
  ...
  const getAllUsers = useCallback(async () => {
    return handleAsync(async () => {
      ...
      if (Array.isArray(response)) {
        console.log('✅ Utilisateurs chargés (array direct):', response.length)
        return response
      }
      
      if (response && Array.isArray(response.data)) {
        console.log('✅ Utilisateurs chargés (response.data):', response.data.length)
        return response.data
      }
      
      if (response && Array.isArray(response.users)) {
        console.log('✅ Utilisateurs chargés (response.users):', response.users.length)
        return response.users
      }
      
      console.warn('⚠️ Format de réponse utilisateurs inattendu:', response)
      return []
    }, 'Erreur lors du chargement des utilisateurs')
  }, [handleAsync])

  const createUserWithEmail = useCallback(async (userData: AdminUserCreationRequest) => {
    const validationErrors = FrontendValidators.validateAdminUserCreationRequest(userData)
    ...
    return handleAsync(async () => {
      const response = await apiClient.post(API_ROUTES.ADMIN.CREATE_USER, userData)
      return response
  ...
  const toggleUserStatus = useCallback(async (userId: number) => {
    return handleAsync(async () => {
      const response = await apiClient.put(API_ROUTES.ADMIN.TOGGLE_USER_STATUS(userId.toString()), {})
      return response
  ...
  const deleteUser = useCallback(async (userId: number) => {
    return handleAsync(async () => {
      const response = await apiClient.delete(API_ROUTES.ADMIN.DELETE_USER(userId.toString()))
      return response
  ...
  const getComplaints = useCallback(async (): Promise<ComplaintData[] | null> => {
    return handleAsync(async () => {
      const response: any = await apiClient.get(API_ROUTES.COMPLAINT.INDEX)
      console.log('🔍 Response from backend:', response)
      
      // Le backend retourne { complaints: [] }
      if (response && Array.isArray(response.complaints)) {
        console.log('✅ Réclamations chargées:', response.complaints.length)
        return response.complaints
      } 
      
      // Fallback : si la réponse est directement un array
      if (Array.isArray(response)) {
        console.log('✅ Réclamations chargées (array direct):', response.length)
        return response
      }
      
      // Dernière chance : response.data
      if (response && Array.isArray(response.data)) {
        console.log('✅ Réclamations chargées (response.data):', response.data.length)
        return response.data
      }
  ...
  const getComplaint = useCallback(async (id: number): Promise<ComplaintData | null> => {
    return handleAsync(async () => {
      const response: any = await apiClient.get(API_ROUTES.COMPLAINT.BY_ID(id.toString()))
  ...
  const updateComplaint = useCallback(async (id: number, data: UpdateComplaintRequest) => {
    const validationErrors = FrontendValidators.validateUpdateComplaintRequest(data)
    if (validationErrors.length > 0) {
      setError(validationErrors.join(', '))
      return null
    }
    ...
    return handleAsync(async () => {
      const response = await apiClient.put(API_ROUTES.COMPLAINT.UPDATE(id.toString()), data)
      return response
    }, 'Erreur lors de la mise à jour de la réclamation')
  }, [handleAsync])

  const resolveComplaint = useCallback(async (id: number, adminNotes?: string) => {
  ...
  const closeComplaint = useCallback(async (id: number, adminNotes?: string) => {
    return updateComplaint(id, { 
      status: 'closed', 
      admin_notes: adminNotes 
    })
  ...
  const setComplaintInProgress = useCallback(async (id: number, adminNotes?: string) => {
    return updateComplaint(id, { 
      status: 'in_progress', 
      admin_notes: adminNotes 
    })
  ...
  const deleteComplaint = useCallback(async (id: number) => {
    return handleAsync(async () => {
      const response = await apiClient.delete(API_ROUTES.COMPLAINT.DELETE(id.toString()))
      return response
  ...
  const getAllPrestataires = useCallback(async () => {
    return handleAsync(async () => {
      try {
        ...
        const response = await apiClient.get(API_ROUTES.SERVICE_PROVIDER.INDEX)
        return Array.isArray(response) ? response : 
               Array.isArray(response.data) ? response.data : []
      } catch (err) {
        console.warn('Prestataires non disponibles, retour tableau vide')
        return []
      }
  ...
  const getAllServiceTypes = useCallback(async () => {
    return handleAsync(async () => {
      try {
        const response = await apiClient.get(API_ROUTES.SERVICE_TYPE.INDEX)
        return Array.isArray(response) ? response : 
               Array.isArray(response.data) ? response.data : []
      } catch (err) {
        console.warn('Types de services non disponibles, retour tableau vide')
        return []
      }
  ...
  const validateService = useCallback(async (id: number, validationData: any) => {
    console.warn('validateService: Fonctionnalité pas encore implémentée')
    return null
  }, [])

  const validatePrestataire = useCallback(async (id: number, validationData: any) => {
    console.warn('validatePrestataire: Fonctionnalité pas encore implémentée')
    return null
  }, [])

  const toggleServiceStatus = useCallback(async (id: number) => {
    console.warn('toggleServiceStatus: Fonctionnalité pas encore implémentée')
    return null
  }, [])

  const generateMonthlyInvoices = useCallback(async (month: number, year: number) => {
    console.warn('generateMonthlyInvoices: Fonctionnalité pas encore implémentée')
    return null
  }, [])
